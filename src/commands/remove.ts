...
export function createRemoveCommand(): Command {
  return new Command('remove')
    ...
    .action(async (skillIdOrName: string, options) => {
      ...
      if (options.claude || options.codex || options.openclaw || options.agents) {
        ...
        for (const skill of skillsToRemove) {
          removeSkill(skill.path, skill.id, skill.name, true);
        }
      ...
      if (!skill) {
        const nameMatches = index.skills.filter(s =>
          s.name.toLowerCase() === skillIdOrName.toLowerCase()
        );

        if (nameMatches.length === 0) {
          // Partial match as fallback
          const partialMatches = index.skills.filter(s =>
            s.name.toLowerCase().includes(skillIdOrName.toLowerCase())
          );
